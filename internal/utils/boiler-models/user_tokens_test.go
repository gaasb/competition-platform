...
func testUserTokensExists(t *testing.T) {
	...
	e, err := UserTokenExists(ctx, tx, o.UserID)
	if err != nil {
...
func testUserTokensFind(t *testing.T) {
	...
	userTokenFound, err := FindUserToken(ctx, tx, o.UserID)
	if err != nil {
		t.Error(err)
	}
...
func testUserTokenToOneUserAccountUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
...
func testUserTokenToOneSetOpUserAccountUsingUser(t *testing.T) {
	var err error
	...
	for i, x := range []*UserAccount{&b, &c} {
		...
		if exists, err := UserTokenExists(ctx, tx, a.UserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	userTokenDBTypes = map[string]string{`UserID`: `bigint`, `AccessToken`: `character varying`, `RefreshToken`: `character varying`, `ExpireAt`: `timestamp without time zone`}
	_                = bytes.MinRead
)
